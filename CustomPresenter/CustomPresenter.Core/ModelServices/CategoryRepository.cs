    ...
    public class CategoryRepository : ICategoryRepository
    {
        ...
        private Category CreateCategory(XElement x)
        {
            ...
            if (!string.IsNullOrEmpty(category.Image))
                category.LargeImage = typeof(ItemRepository).Assembly.GetManifestResourceStream("CustomPresenter.Core.Assets.Images.Category." + category.Image).ToByte();
        ...
        public virtual IEnumerable<Category> GetAll()
        {
            if (_items == null)
            {
                XDocument doc = XDocument.Load(typeof(CategoryRepository).Assembly.GetManifestResourceStream("CustomPresenter.Core.Assets.Data.Categories.xml"));
